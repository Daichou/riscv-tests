...
fname="${1%.*}"
fsize=`stat --printf="%s" $fname.riscv`
fsize=`printf "%08x\n" $fsize`
LD_src=./aquila_common/test.ld
# The start addr of the executable is extracted from the linker script
start=`sed -e 's/^[ \t]*//' $LD_src | grep ^code_ram | cut -d= -f2 | cut -d, -f1 | sed -e 's/^[ \t]*//' | cut -c 3-`

echo $start | xxd -r -p | xxd -e -g4 | xxd -r > _header_.bin
echo $fsize | xxd -r -p | xxd -e -g4 | xxd -r >> _header_.bin
cat _header_.bin $fname.riscv > $fname.ebf
rm -f _header_.bin
